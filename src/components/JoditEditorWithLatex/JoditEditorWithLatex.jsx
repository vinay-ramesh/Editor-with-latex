...
//                     scale: 3, // Higher quality for better dimensions
//                     useCORS: true,
//                     allowTaint: true,
//                     logging: false,
...
//                     editorInstance.s.insertHTML(`<img src="${imageDataUrl}" data-latex="${latexCode}" alt="LaTeX: ${latexCode}" title="LaTeX Formula" style="display: inline-block; vertical-align: middle; max-width: 100%;" />`);
...
//                 });
//             }, 300); // More time for fonts to load
...
const JoditEditorWithLatex = () => {
    ...
    const insertLatexAsImage = (editorInstance, latexCode) => {
        try {
            // Render LaTeX to HTML with inline mode for better alignment
            const html = katex.renderToString(latexCode, {
                throwOnError: false,
                displayMode: false // Use inline mode for better vertical alignment
            });
            ...
            setTimeout(() => {
                html2canvas(tempDiv, {
                    ...
                    // Get current cursor position and insert without changing it
                    const currentNode = editorInstance.s.current();
                    ...
                    img.setAttribute('title', 'LaTeX Formula');
                    img.style.cssText = 'display: inline; vertical-align: middle; max-width: 50%;';

                    // Insert at current cursor position
                    editorInstance.s.insertNode(img, false);

                    // Move cursor after the image
                    range.setStartAfter(img);
                    range.collapse(true);
                    editorInstance.s.selectRange(range);
